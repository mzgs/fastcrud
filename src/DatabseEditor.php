    ...
    private static function renderHtml(array $tables, array $tableColumns, string $driver): string
    {
        ...
        if ($tables === []) {
            ...
            foreach ($tables as $index => $table) {
                ...
                $html .= '<div>';
                $html .= '<h3 class="h5 mb-1">' . $tableEscaped . '</h3>';
                $html .= '<p class="text-muted mb-0">Manage structure, rename, or add new columns.</p>';
                $html .= '</div>';
                $html .= '</div>';
                ...
                $html .= '<form method="post" class="row g-2 align-items-end mb-4" data-fc-db-editor-form>';
                ...
                $html .= '<div class="col-md-8">';
                $html .= '<label class="form-label text-muted">Rename table</label>';
                $html .= '<input type="text" name="new_table_name" class="form-control" placeholder="New table name" required pattern="[A-Za-z0-9_]+">';
                $html .= '</div>';
                $html .= '<div class="col-md-4">';
                $html .= '<button type="submit" class="btn btn-outline-primary w-100"><i class="bi bi-arrow-repeat me-1"></i>Rename</button>';
                $html .= '</div>';
                $html .= '</form>';
